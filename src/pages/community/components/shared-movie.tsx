...
interface TSharedMovieProps {
  id: string;
}

const SharedMovie = ({ id }: TSharedMovieProps) => {
  const movieIdNum = Number(id);
  const navigate = useNavigate();
  const lang = useRegionStore((state) => state.language);
  const { data, isFetching } = useMovieDetailsQuery(movieIdNum, lang);
          ...
          <div>
            <S.MovieGenreList>
              {data?.genres.map((genre) => {
                return <S.MovieGenreItem key={genre.name}>{genre.name}</S.MovieGenreItem>;
              })}
            </S.MovieGenreList>
